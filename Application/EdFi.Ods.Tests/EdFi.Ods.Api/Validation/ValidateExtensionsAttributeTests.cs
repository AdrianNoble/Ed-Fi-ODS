...
namespace EdFi.Ods.Tests.EdFi.Ods.Api.Validation
{
    ...
    public class ValidateExtensionsAttributeTests
    {
        ...
        public class When_validating_an_object_with_an_invalid_entity_extension
            : TestFixtureBase
        {
            private ICollection<ValidationResult> _actualResults;
                ...
                _actualResults = new DataAnnotationsEntityValidator()
                    .ValidateObject(
                        new TestObject
                        {
                            ValidatedDictionary = new Hashtable
                            {
                                {"TestSchema", new List<TestObjectExtension> { new TestObjectExtension { Value=1} }}
                            }
                        });
            }
            ...
            [Assert]
            public void Should_indicate_that_the_extension_object_from_TestSchema_was_invalid()
            {
                var compositeResults = (_actualResults.Single() as CompositeValidationResult).Results;

                Assert.That(compositeResults.Single().ErrorMessage, Is.EqualTo("TestObjectExtension (TestSchema): The Value field is required."));
            }
        }
    }
}
